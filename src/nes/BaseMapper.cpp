...
inline void BaseMapper::set_prg_rom(std::vector<uint8_t>&& vec) {
  prg = std::move(vec);
}

inline void BaseMapper::set_chr_rom(std::vector<uint8_t>&& vec) {
  chr = std::move(vec);
}

inline void BaseMapper::set_prg_ram(std::vector<uint8_t>&& vec) {
  prg_ram = std::move(vec);
}

inline std::vector<uint8_t> BaseMapper::get_prg_ram() const {
  return prg_ram;
}
...
inline uint8_t BaseMapper::chr_read(uint16_t addr) const {
  size_t slot     = addr / 0x400;
  size_t chr_addr = addr % 0x400;
  return chr[chr_map[slot] + chr_addr];
}

inline void BaseMapper::prg_write(uint16_t addr, uint8_t value) {
  prg_ram[addr] = value;
}

inline void BaseMapper::chr_write(uint16_t addr, uint8_t value) {
  chr[addr] = value;
}
