    ...
    void update_mirroring() {
        switch (mapper->getNameTableMirroring()) {
            case HORIZONTAL:
                ...
                NES_DEBUG("Horizontal Name Table mirroring set. (Vertical Scrolling)");
                break;
            case VERTICAL:
                name_tables[0] = name_tables[2] = 0;
                name_tables[1] = name_tables[3] = 0x400;
                NES_DEBUG("Vertical Name Table mirroring set. (Horizontal Scrolling)");
                break;
            case ONE_SCREEN_LOWER:
                name_tables[0] = name_tables[1] = name_tables[2] = name_tables[3] = 0;
                NES_DEBUG("Single Screen mirroring set with lower bank.");
                break;
            case ONE_SCREEN_HIGHER:
                name_tables[0] = name_tables[1] = name_tables[2] = name_tables[3] = 0x400;
                NES_DEBUG("Single Screen mirroring set with higher bank.");
                break;
            default:
                name_tables[0] = name_tables[1] = name_tables[2] = name_tables[3] = 0;
                NES_DEBUG("Unsupported Name Table mirroring : " << mapper->getNameTableMirroring());
        }
    }
